...
var _ machinery.Template = &MakefileHelp{}

type MakefileHelp struct {
	machinery.TemplateMixin
	machinery.ComponentConfigMixin
}

func (m *MakefileHelp) SetTemplateDefaults() error {
	if m.Path == "" {
		m.Path = "Makefile_help.awk"
	}

	m.TemplateBody = makefileHelpTemplate
	m.IfExistsAction = machinery.Error

	return nil
}

const makefileHelpTemplate = `#!/usr/bin/env awk -f
